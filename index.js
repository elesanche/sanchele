...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '❬❗❭ banca',
				success: '️❬ ✔ ❭ asi se hace wachin 🖤',
				error: {
					stick: 'hacelo bien pendejo ^_^',
					Iv: 'link invalido☹️'
				},
				only: {
					group: '❬❗❭ solo en grupo ',
					ownerG: '❬❗❭ el unico autorizado es antin ',
					ownerB: '❬❗❭  el unico autorizado es antin ',
					admin: '❬❗❭ no conoces la jerarquia',
					Badmin: '❬❗❭ necesito admin paparulo '
				}
			}
          ...
          case 'ping':
            ...
            case 'kangcopet':
                ...
                case 'hidetag':
					...
					if (!isOwner) return reply('sos antin?')
                ...
                case 'quotemaker':
                 ...
                 case 'phlogo':
					...
					break
                case 'verdad':
					const trut =['alguna vez te gusto alguien? cuanto tiempo?','harias un trio con zuru?(puede ser zuru hombre o zuru mujer)','cual es tu mayor miedo?','te cojerias a paula rojas y al perro?',']
					const ttrth = trut[Math.floor(Math.random() * trut.length)]
					truteh = await getBuffer(`https://i.ibb.co/305yt26/bf84f20635dedd5dde31e7e5b6983ae9.jpg`)
					client.sendMessage(from, truteh, image, { caption: '*Truth*\n\n'+ ttrth, quoted: mek })
					break
				case 'reto':
					const dare =['re aburrido esto ni ganas de traducir nose hacete una paja con la gladys y intenta durar mas de un minuto']
                ...
                case 'bug':
                     const pesan = body.slice(5)
                      if (pesan.length > 300) return client.sendMessage(from, 'Muy largo el texto pibe', msgType.text, {quoted: mek})
				...
				case 'chord':
					...
					break
                case 'letra':
                    if (args.length < 1) return reply('donde esta el titulo de la cancion papa')
                    teha = body.slice(7)
                ...
                case 'perro':
					...
					break
                case 'spamcall':
                   if (args.length < 1) return ('pone el numero wachin')
                   weha = body.slice(10)
                ...
                case 'indohot':
                   ...
                   break
				case 'ytmp4':
					if (args.length < 1) return reply('donde esta el url eh?, y lo barbijo?')
                ...
                case 'text3d':
                    ...
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return client.sendMessage(from, 'el texto es largo, máximo 10 frase', text, {quoted: mek})
				...
				break
                case 'kbbi':
					if (args.length < 1) return reply('es un diccionario indio no hay un choto')
					anu = await fetchJson(`https://mnazria.herokuapp.com/api/kbbi?search=${body.slice(6)}`, {method: 'get'})
					reply('Menurut Kbbi:\n\n'+anu.result)
					break
                case 'nombresignificado':
					if (args.length < 1) return reply('que quere buscar?')
				...
				case 'ocr': 
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('enviar foto con ${prefix}ocr')
					}
					break
				case 'stiker': 
				case 'sticker':
            ...
            break	
				...
				case 'tts':
					...
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('es el texto que significa bambang')
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							fs.unlinkSync(ranm)
							buff = fs.readFileSync(rano)
							if (err) return reply('si erre ;(, volve a intentar ^_^')
				...
				case 'tiktokstalk':
					try {
						...
						reply('[𝗘𝗥𝗥𝗢𝗥] seguro lo pusiste mal salamin')
					}
					break
				case 'fitnah':	
				case 'fake':          
               if (!isGroup) return reply(mess.only.group)
				...
				case 'clearall':
					...
					reply('limpiado todo con exito papa  :)')
					break
			       case 'block':
				...
				if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
				await client.leaveGroup(from, 'chau', groupId)
                    break
				case 'bc': 
					if (!isOwner) return reply(' *sos antin* ?') 
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('queres agregar al barba?')
					if (args[0].startsWith('08')) return reply('usa el codigo de pais mas')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('no se pudo agregar al pibe ')
					}
					break
					case 'grup':
					case 'group':
            ...
            case 'owner':
            case 'creator':
                  client.sendMessage(from, {displayname: "capece", vcard: vcard}, MessageType.contact, { quoted: mek})
       client.sendMessage(from, 'este es mi numero de propietario',MessageType.text, { quoted: mek} )
           break    
           case 'setname':
                if (!isGroup) return reply(mess.only.group)
			    if (!isGroupAdmins) return reply(mess.only.admin)
                ...
                client.sendMessage(from, 'impecable patron', text, {quoted: mek})
                break
                case 'setdesc':
                if (!isGroup) return reply(mess.only.group)
			    if (!isGroupAdmins) return reply(mess.only.admin)
           ...
           case 'demote':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `𝗮𝗵𝗼𝗿𝗮 𝘀𝗼𝘀 𝘂𝗻 𝘃𝗶𝗹𝗹𝗲𝗿𝗶𝘁𝗼 𝗱𝗲𝗹 𝗽𝗮𝘁𝗶𝗼 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`𝗮𝗵𝗼𝗿𝗮 @${mentioned[0].split('@')[0]} 𝘀𝗼𝘀 𝘂𝗻 𝘃𝗶𝗹𝗹𝗲𝗿𝗶𝘁𝗼 𝗱𝗲𝗹 𝗽𝗮𝘁𝗶𝗼`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `𝘁𝗲 𝗽𝗼𝗱𝗲𝘀 𝘃𝗲𝗻𝗶𝗿 𝗮 𝘃𝗶𝘃𝗶𝗿 𝗰𝗼𝗻 𝗻𝗼𝘀𝗼𝘁𝗿𝗼𝘀 𝗮𝗹 𝗽𝗮𝗯𝗲𝗹𝗹𝗼𝗻(+_+) :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`𝘁𝗲 @${mentioned[0].split('@')[0]} 𝗽𝗼𝗱𝗲𝘀 𝘃𝗲𝗻𝗶𝗿 𝗮 𝘃𝗶𝘃𝗶𝗿 𝗰𝗼𝗻 𝗻𝗼𝘀𝗼𝘁𝗿𝗼𝘀 𝗮𝗹 𝗽𝗮𝗯𝗲𝗹𝗹𝗼𝗻(+_+)`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`𝗺𝗲 𝗰𝗮𝗶𝗮𝘀 𝗯𝗶𝗲𝗻 @${mentioned[0].split('@')[0]} `, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('oli mal ;(, proba otra vez ^_^')
				...
				case 'simih':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('perro')
					if (Number(args[0]) === 1) {
						if (isSimi) return reply('activado !!!')
						samih.push(from)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('❬ 𝗲𝘅𝗶𝘁𝗼 ❭ 𝗮𝗰𝘁𝗶𝘃𝗮𝗿 𝗹𝗮 𝗳𝘂𝗻𝗰𝗶𝗼́𝗻 𝘀𝗶𝗺𝗶 𝗲𝗻 𝗲𝘀𝘁𝗲 𝗴𝗿𝘂𝗽𝗼')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('❬ 𝗲𝘅𝗶𝘁𝗼 ❭ 𝗮𝗰𝘁𝗶𝘃𝗮𝗿 𝗹𝗮 𝗳𝘂𝗻𝗰𝗶𝗼́𝗻 𝘀𝗶𝗺𝗶 𝗲𝗻 𝗲𝘀𝘁𝗲 𝗴𝗿𝘂𝗽𝗼')
					} else {
						reply(' *escribi el comando 1 para activar, 0 para desactivar* \n𝗰𝗼𝗻𝘁𝗼𝗵: 𝘀𝗶𝗺𝗶𝗵 𝟭')
					}
					break
				case 'nsfw':
					if (!isGroup) return reply(mess.only.group)
					...
					if (args.length < 1) return reply('perro')
					if (Number(args[0]) === 1) {
						if (isNsfw) return reply('ya activo??f !!')
						nsfw.push(from)
						fs.writeFileSync('./src/nsfw.json', JSON.stringify(nsfw))
						reply('❬ 𝗲𝘅𝗶𝘁𝗼 ❭ 𝗮𝗰𝘁𝗶𝘃𝗮𝗿 𝗹𝗮 𝗳𝘂𝗻𝗰𝗶𝗼́𝗻 𝗻𝘀𝗳𝘄 𝗲𝗻 𝗲𝘀𝘁𝗲 𝗴𝗿𝘂𝗽𝗼')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./src/nsfw.json', JSON.stringify(nsfw))
						reply('❬ 𝗲𝘅𝗶𝘁𝗼 ❭ 𝗮𝗰𝘁𝗶𝘃𝗮𝗿 𝗹𝗮 𝗳𝘂𝗻𝗰𝗶𝗼́𝗻 𝗻𝘀𝗳𝘄 𝗲𝗻 𝗲𝘀𝘁𝗲 𝗴𝗿𝘂𝗽𝗼'')
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ 𝗲𝘅𝗶𝘁𝗼 ❭ 𝗮𝗰𝘁𝗶𝘃𝗮𝗿 𝗹𝗮 𝗳𝘂𝗻𝗰𝗶𝗼́𝗻 𝘄𝗲𝗹𝗰𝗼𝗺𝗲/𝗹𝗲𝗳𝘁  𝗲𝗻 𝗲𝘀𝘁𝗲 𝗴𝗿𝘂𝗽𝗼'')
					} else if (Number(args[0]) === 0) {
				...
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(' *lo conozco a usted* ?') 
					if (args.length < 1) return reply(' *etiqueta a quien queres poner!!!* ')
					...
					try {
						...
						reply(' *oli mal ;(, proba otra vez ^_^* ')
					}
					break
				case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
